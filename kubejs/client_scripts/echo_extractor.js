...
ItemEvents.tooltip(event => {
    ...
    event.addAdvanced(['kubejs:echo_extractor'], (item, advanced, text) => {
        // Line 1: Lore
        text.add(1, Text.of("用于从刷怪笼中提取回响的设备").gray().italic());

            // --- SECTION 1: EXTRACTION EXPERTISE (NOW FIRST) ---
            text.add(2, Text.of("提取专精：").white());
            ...
            text.add(3, [Text.of('§7专精等级：'), Text.of(`${mastery.toFixed(1)} `), Text.of('§8| '), Text.of(`§bTier ${bonusTier}`)]);
            text.add(4, [Text.of('§7反噬风险：'), Text.of(`${riskColor}${currentRisk.toFixed(1)}%`)]);
            ...
            text.add(5, [Text.of('§7层级进度：'), Text.of(`§8[${bar}§8] §f${mastery.toFixed(1)}/${nextTierGoal}`)]);

        if (!event.isShift()) {
            text.add(6, Text.of("按住[Shift]查看更多信息").yellow());
        } else {
            // --- SECTION 2: MECHANICS (NOW SECOND) ---
            text.add(6, Text.of(' ')); // Spacer
            text.add(7, [Text.of("§6• "), Text.of("从目标共振方块中抽取精华")]);
            text.add(8, [Text.of("§b• "), Text.of("获得一个§d密封古卷§r，内含主题战利品")]);
            text.add(9, [Text.of("§2• "), Text.of(`Heavy slowness, §e${castSec}s§r cast, §e${cooldownSec}s§r cooldown`)]);
            text.add(10, [Text.of("§4• "), Text.of("低专精会增加§c时序反噬§r的风险")]);
            text.add(11, [Text.of("§4• "), Text.of("有5%的永久失败几率")]);
    ...
    event.addAdvanced('kubejs:sealed_tome', (item, assistant, text) => {
        // Line 1: Lore
        text.add(1, Text.of("一本充满失传时代低语的书").gray().italic());

        if (!event.shift) {
            text.add(2, Text.of("按住[Shift]感知其中内容。").yellow());
        } else {
            text.add(2, Text.of("机制：").white());
            ...
            text.add(4, Text.of(" 2. §6用法：§r 右键打破封印并领取内容。"));
            text.add(5, Text.of(" 3. §d奖励：§r 可能包含该主题的稀有神器或材料。"));
        }
    });	
});
